...
public class RadioController : MonoBehaviour
{
    ...
    private const float SyncSeekToleranceSeconds = 0.25f;
    private const float SyncReseekIntervalSeconds = 1.0f;
    private const float SyncSeekVerifySeconds = 0.05f;
    private const float SyncSeekMinTrackAgeSeconds = 2f;
    private const float ShortTrackMaxSeconds = 3f;
    /// <summary>Tiny MP3s often fail Unity decode with compressed-in-memory — decompress on load instead.</summary>
    private const int SmallCacheFileBytes = 128 * 1024;
    private const float SyncScheduledLeadSeconds = 0.15f;
    private const int MinBytesBeforePlay = 100000;
    private const float EndOfTrackEpsilonSeconds = 0.2f;
    private static string RadioCacheDirectory =>
        Path.Combine(Application.temporaryCachePath, "FlamiePracRadio");
    ...
    private string lastResyncTrackId = string.Empty;
    private double lastResyncStartServerTime = double.NaN;
    private float lastResyncDuration = -1f;
    private bool lastResyncClockStarted;
    private float syncedClockStartedAtUnscaled = -1f;
    ...
    /// <summary>Remove cached phlstats MP3s from Unity temp (FlamiePracRadio).</summary>
    public static void ClearRadioDiskCache()
    {
        try
        {
            ...
            Directory.Delete(RadioCacheDirectory, recursive: true);
            FlamieLog.Info("[FlamiePrac] Radio disk cache cleared.");
        }
        catch (Exception ex)
        {
            FlamieLog.Warn("[FlamiePrac] Radio disk cache clear failed: " + ex.Message);
        }
    }
    ...
    private void RecreatePlaybackSource()
    {
        if (playback != null)
        {
            try
        ...
        bool clockStarted)
    {
        ...
        else if (duration > 0.05f && duration + 0.75f < syncedDuration)
        {
            float playbackTime = playback != null ? playback.time : 0f;
            if (playbackTime <= duration + 1f)
                syncedDuration = duration;
        }
        else if (duration > 0.05f && syncedDuration < 0.05f)
            syncedDuration = duration;
        ...
        if (trackChanged)
        {
            ...
            ClearResyncFingerprint();
            syncedClockStartedAtUnscaled = -1f;
            syncedDuration = ResolveDurationForTrack(trackId, duration);
            currentSong = index;
            pendingStartIndex = -1;
            RecreatePlaybackSource();
        ...
        else if (!clockStarted)
        {
            ...
            else if (!IsLoadingSyncedTrack(index))
            {
                RequestPlay(index, recordHistory: false);
            }
        }
        else if (clockJustStarted)
        {
            ...
            nextEndReportRetryTime = 0f;
            nextSyncSeekTime = 0f;
            syncedClockStartedAtUnscaled = Time.unscaledTime;
            SetStatus(string.Empty);
            if (!IsPlaybackClipForSyncedTrack() && !IsLoadingSyncedTrack(index))
                RequestPlay(index, recordHistory: false);
            TrySyncSeek(force: true);
            RememberResyncFingerprint(trackId, startServerTime, syncedDuration, clockStarted);
            ApplyListeningState(seekIfOn: true);
        }
        else if (ShouldResyncSeek(trackId, startServerTime, syncedDuration, clockStarted))
        {
    ...
    private void ClearResyncFingerprint()
    {
        lastResyncTrackId = string.Empty;
        lastResyncStartServerTime = double.NaN;
        lastResyncDuration = -1f;
        lastResyncClockStarted = false;
    }

    private void RememberResyncFingerprint(string trackId, double startServerTime, float duration, bool clockStarted)
    {
        lastResyncTrackId = trackId ?? string.Empty;
        lastResyncStartServerTime = startServerTime;
        lastResyncDuration = duration;
        lastResyncClockStarted = clockStarted;
    }

    private bool ShouldResyncSeek(string trackId, double startServerTime, float duration, bool clockStarted)
    {
        if (!string.Equals(lastResyncTrackId, trackId, StringComparison.Ordinal))
            return true;
        if (lastResyncClockStarted != clockStarted)
            return true;
        if (double.IsNaN(lastResyncStartServerTime) ||
            Math.Abs(lastResyncStartServerTime - startServerTime) > 0.01)
            return true;
        if (Math.Abs(lastResyncDuration - duration) > 0.5f)
            return true;
    ...
    private void TrySyncSeek(bool force)
    {
        ...
        // Block only while a different track is still streaming in.
        if (playRoutine != null && !IsPlaybackClipForSyncedTrack())
            return;
        ...
        if (!force)
        {
            float drift = Mathf.Abs(primary.time - seek);
            if (drift <= SyncSeekToleranceSeconds)
                return;

            float minTrackAge = SyncSeekMinTrackAgeSeconds;
            if (len > 0.05f && len < ShortTrackMaxSeconds)
                minTrackAge = Mathf.Min(minTrackAge, len * 0.45f);

            if (syncedClockStartedAtUnscaled > 0f &&
                Time.unscaledTime - syncedClockStartedAtUnscaled < minTrackAge)
                return;
        }
        else if (!nearEnd && primary.isPlaying &&
        ...
        FlamieLog.InfoThrottled("radio-sync-seek",
            "[FlamiePrac] Radio sync seek '" + CurrentTrackTitle + "' @ " + seek.ToString("0.00") + "s");
    }
        ...
        if (IsSyncedPlayback)
        {
            if (playback.clip == null || !IsPlaybackClipForSyncedTrack())
            {
                ...
                if (!string.IsNullOrEmpty(syncedTrackId))
                {
                    ...
                    if (index >= 0 && !IsLoadingSyncedTrack(index))
                        RequestPlay(index, recordHistory: false);
                }
                return;
            }
    ...
    {
        // Server clock owns playlist advance.
    }
    ...
    private void MaybeReportVerifiedClipDuration()
    {
        ...
        if (clipLen > syncedDuration + 0.05f)
    ...
    {
        if (IsLoadingSyncedTrack(index))
            return;

        CancelPlayRoutine();
        clipAppliedDuringStream = false;
    ...
    {
        if (!pendingStreamApply || pendingStreamApplyIndex < 0)
            return;

        int index = pendingStreamApplyIndex;
        if (tracks == null || index < 0 || index >= tracks.Length || clips == null || clips[index] == null)
            return;

        if (!StillWantsTrack(index, playRequestGeneration))
        {
            pendingStreamApply = false;
            pendingStreamApplyIndex = -1;
            return;
        ...
        clipAppliedDuringStream = true;
        pendingStreamApply = false;
        pendingStreamApplyIndex = -1;
        ApplyClipToOutputs(index, recordHistory: false);
        if (IsSyncedPlayback && syncedClockStarted && listeningEnabled)
        {
            nextSyncSeekTime = 0f;
            TrySyncSeek(force: true);
        }
    ...
    private IEnumerator PlayTrackRoutine(int index, bool recordHistory, int generation)
    {
        ...
        yield return EnsureClipLoaded(index, allowResignRetry: true, generation, recordHistory);
        if (generation != playRequestGeneration &&
            !StillWantsTrack(index, generation))
        {
            advancingTrack = false;
        ...
        {
            FlamieLog.Warn("[FlamiePrac] Song failed to load: " + tracks[index].Title);
            if (IsSyncedPlayback)
            {
    ...
    private bool pendingStreamApply;
    ...
    private void ReleaseActiveStreamHandler()
    {
        ...
        try
        {
            activeStreamHandler.Dispose();
        }
        catch
        {
            // Handler may already be torn down during session exit.
        }
    ...
    private IEnumerator EnsureClipLoaded(int index, bool allowResignRetry, int generation, bool recordHistory)
    {
        if (clips != null && clips[index] != null)
            yield break;
        ...
        clipAppliedDuringStream = false;
        yield return DownloadClipOnce(index, generation, recordHistory);
    ...
    private IEnumerator DownloadClipOnce(int index, int generation, bool recordHistory)
    {
        if (clips != null && clips[index] != null)
            yield break;
        ...
        yield return LoadClipFromDisk(index, entry, generation);
    }

    private static string GetRadioCachePath(string trackId)
    {
        ...
        return Path.Combine(RadioCacheDirectory, safe + ".mp3");
    }
    ...
    private IEnumerator EnsureTrackCachedOnDisk(TrackEntry entry, int index, int generation)
    {
        ...
        {
            // Fall through and try re-download.
        }
    ...
    private IEnumerator LoadClipFromDisk(int index, TrackEntry entry, int generation)
    {
        ...
        string path = !string.IsNullOrEmpty(entry.LocalPath)
            ? Path.GetFullPath(entry.LocalPath)
            : GetRadioCachePath(entry.Id);
        ...
        bool smallFile = fileBytes > 0 && fileBytes < SmallCacheFileBytes;
        bool loaded = false;

        if (smallFile)
        {
            yield return LoadClipFromDiskAttempt(index, entry, generation, path, streamAudio: false, compressed: false);
            loaded = clips != null && index < clips.Length && clips[index] != null;
            if (!loaded)
            {
                yield return LoadClipFromDiskAttempt(index, entry, generation, path, streamAudio: false, compressed: true);
                loaded = clips != null && index < clips.Length && clips[index] != null;
            }
        }
        else
        ...
        if (loaded || !string.IsNullOrEmpty(entry.LocalPath))
            yield break;

        FlamieLog.Error("[FlamiePrac] Failed to load '" + entry.Title + "' from disk after decode attempts.");
    ...
    private IEnumerator LoadClipFromDiskAttempt(
        int index,
        TrackEntry entry,
        int generation,
        string path,
        ...
        bool compressed)
    {
        ...
        var streamHandler = new DownloadHandlerAudioClip(fileUri, AudioType.MPEG)
        {
            streamAudio = streamAudio,
            compressed = compressed
        };

        using (UnityWebRequest www = new UnityWebRequest(fileUri, UnityWebRequest.kHttpVerbGET, streamHandler, null))
        {
            ...
            if (!StillWantsTrack(index, generation))
            {
                DisposeStreamHandler(ref streamHandler);
                yield break;
            }

            string handlerErr = streamHandler.error;
            AudioClip clip = null;
            if (www.result == UnityWebRequest.Result.Success && string.IsNullOrEmpty(handlerErr))
                clip = DownloadHandlerAudioClip.GetContent(www);
            else if (streamHandler.audioClip != null)
                clip = streamHandler.audioClip;
            ...
            FlamieLog.Info("[FlamiePrac] Radio loaded: " + entry.Title + " (" + clip.length.ToString("0.0") + "s" +
                           (compressed ? ", compressed" : ", pcm") + ")");
            ReportStreamDurationIfNeeded(entry.Id, clip.length);
        }
    }

    private static void DisposeStreamHandler(ref DownloadHandlerAudioClip handler)
    {
        if (handler == null)
            return;
    ...
    private bool StillWantsTrack(int index, int generation)
    {
        if (generation == playRequestGeneration)
            return true;
        ...
        return entry != null &&
               ...
               string.Equals(entry.Id, syncedTrackId, StringComparison.Ordinal);
    }

    private void ReportStreamDurationIfNeeded(string trackId, float duration)
    {
        if (string.IsNullOrEmpty(trackId) || duration <= 0.05f)
            return;

        if (!IsSyncedPlayback || !string.Equals(trackId, syncedTrackId, StringComparison.Ordinal))
            return;

        RememberVerifiedDuration(trackId, duration);
        if (reportedDurationForCurrent &&
            verifiedDurations.TryGetValue(trackId, out float existing) &&
            Mathf.Abs(existing - duration) < 0.5f)
        {
            return;
        }
        ...
        syncedDuration = duration;
        reportedDurationForCurrent = true;
        RadioSync.ClientReportDuration(trackId, duration);
    }
    ...
    {
        if (!IsSyncedPlayback || string.IsNullOrEmpty(syncedTrackId) || duration <= 0.05f)
            return;
    ...
    private void ApplyClipToOutputs(int index, bool recordHistory)
    {
        ...
        if (clip != null && clip.length > 0.05f)
        {
            if (IsSyncedPlayback && (IsWaitingForServerStart() || seek < 0f))
            {
                ...
                playback.Pause();
                ReportTrackReadyIfNeeded();
                FlamieLog.Info("[FlamiePrac] Radio prepared: " + tracks[index].Title +
                               " (" + clip.length.ToString("0.0") + "s, waiting for server clock)");
                NotifyStateChanged();
                return;
            }

            // If sync clock is already past the clip, hold at end and nudge the server playlist.
            if (IsSyncedPlayback && seek >= clip.length - EndOfTrackEpsilonSeconds)
            {
                ...
                playback.time = endPos;
                playback.Pause();
                FlamieLog.Info("[FlamiePrac] Radio missed (late): " + tracks[index].Title +
                               " seek=" + seek.ToString("0.0") + "s clip=" + clip.length.ToString("0.0") + "s");
                NotifyStateChanged();
                return;
            }
        ...
        if (IsSyncedPlayback)
        {
            CancelTrackEndWatch();
            if (clip != null && clip.length > 0.05f)
                ReportStreamDurationIfNeeded(tracks[index].Id, clip.length);
        }
        else if (listeningEnabled)
        {
    ...
    private IEnumerator PlayAlternateRoutine(int index, int attemptsLeft, int generation)
    {
        while (attemptsLeft > 0)
        {
            ...
            yield return EnsureClipLoaded(index, allowResignRetry: true, generation, recordHistory: true);
            if (generation != playRequestGeneration)
                yield break;
