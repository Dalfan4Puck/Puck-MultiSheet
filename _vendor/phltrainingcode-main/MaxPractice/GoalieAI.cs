...
namespace MaxPractice
{
    public class GoalieAI : MonoBehaviour
    {
        ...
        private float postDashStandDuration = 0.35f;
        private bool isBrakingOvershoot = false;
        private const float MaxGoalieHeight = 0.55f;
        ...
        private void ApplyTrainingGoalOverrides()
        {
            if (FlamiePracGoaliePlacement.TryGetGoalPos(PlayerTeam.Red, out Vector3 red))
                redGoalPos = red;
            if (FlamiePracGoaliePlacement.TryGetGoalPos(PlayerTeam.Blue, out Vector3 blue))
                blueGoalPos = blue;
        }
        ...
        private void FixedUpdate()
        {
            ...
            try
            {
                ...
                // Climbed the cage / launched onto the net — drop back into the crease on ice.
                if (currentPos.y > MaxGoalieHeight)
                {
                    TeleportToCrease();
                    return;
                }
                ...
                if (puckInZone && distToGoal < effectiveAggressionRange)
                {
                    ...
                    float minZ, maxZ;
                    if (isRedTeam) { minZ = goalPos.z + 0.55f; maxZ = goalPos.z + 3f; }
                    else { minZ = goalPos.z - 3f; maxZ = goalPos.z - 0.55f; }
                    ...
                    if (distToPuck < butterflyDistance)
                    {
                        ...
                        bool needsCrabDash = Mathf.Abs(lateralX) > 0.85f;
                    ...
                    else
                    {
                        ...
                        bool isPostDashStand = (Time.time - lastDashTime) < postDashStandDuration;
                        bool needsCrabDash = Mathf.Abs(lateralX) > 1.45f;
                        bool isSliding = false;
                        try { isSliding = body.IsSliding.Value; } catch { }
        ...
        private void TeleportToCrease()
        {
            try
            {
                Vector3 resetPos;
                Quaternion resetRot;
                if (FlamiePracGoaliePlacement.TryGetCrease(team, out resetPos, out resetRot))
                {
                    // ok
                }
                else
                {
                    resetPos = goalPos;
                    resetPos.z += isRedTeam ? 1.85f : -1.85f;
                    resetPos.y = 0f;
                    resetPos.x = 0f;
        ...
        private void ReturnToCenter(Vector3 currentPos, Vector3 gPos, bool redTeam, float xBias = 0f)
        {
            ...
            try
            {
                ...
                if (FlamiePracGoaliePlacement.TryGetCrease(team, out Vector3 crease, out _))
                    targetCenter = crease;
                else
                ...
                else if (Mathf.Abs(lateralDist) > 1.1f)
                {
                    try { playerInput.SlideInput.ServerValue = true; } catch { }
                    if (isSliding && Time.time - lastDashTime > dashCooldown)
                    {
